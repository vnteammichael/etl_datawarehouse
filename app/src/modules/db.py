...
class Database:
    ...
    def load_data(self, query, data):
        ...
        with self.conn.cursor() as cur:
            ...
            cur.close()
            return cur.rowcount
    def load_data_batch(self, queryText, data, tableName):
        """Load data to warehouse"""
    ...
    def load_data_bulk(self, part_query, format_str, data, on_conflict):
        """Load data to warehouse"""
        self.connect()
        with self.conn.cursor() as cur:
            args = [cur.mogrify(format_str, x).decode('utf-8')
                    for x in data]
            args_str = ', '.join(args)
            cur.execute(part_query + " "
                        + args_str + " " + on_conflict)
            self.conn.commit()
            cur.close()
            return cur.rowcount
    # def update_data_bulk(self, query, data):
    ...
    # Nested the query inside a SELECT to get the correct format and order
    # Missing field will be replace with NULL (for context) or 0 (for values)
    # The format may change from games to games or with latter version
    # TODO: separate the required list from current fix query string --> put in a schema config file
    ...
    def load_fact_snapshot(self, db_clickhouse, action, sql_query = "", df = None):
        ...
        for _, row in df.iterrows():
            market_id = platform_id = social_id = dimension_1_id = dimension_2_id = dimension_3_id = None
            ...
            try:
                ...
                market_id = self.load_market(row['market'])
                platform_id = self.load_platform(row['platform'])
                social_id = self.load_social(row['social'])
            ...
            row_added = [date_id, market_id, platform_id, social_id, metric_id, dimension_1_id, dimension_2_id, dimension_3_id, row['metric_value'], row['metric_value_2']]

            fact_snapshot.append(tuple(row_added))

        query = 'INSERT INTO fact_snapshot (date_id, market_id, platform_id, social_id, metric_id, dimension_1_id, dimension_2_id, dimension_3_id, metric_value, metric_value_2) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)'
        self.connect()
        try:
            with self.conn.cursor() as cur:
    ...
    def load_market(self, market_name):
        """
        It checks if the market name is in the cache, if not it checks if it's in
        ...
        # Cache
        if (self.cache.get('market') != None and self.cache.get('market').get(market_name) != None):
            return self.cache.get('market').get(market_name)


        self.connect()
        query = ("""
            SELECT id FROM dim_market WHERE market_name=%s LIMIT 1;
        """)
        with self.conn.cursor(cursor_factory=DictCursor) as cur:
            cur.execute(query, (market_name,))
            row = cur.fetchone()
            if row:
                id = row["id"]
            else:
                cur.execute(dim_market_table_insert, (market_name,))
                self.conn.commit()
                id = cur.fetchone()[0]
        cur.close()


        if self.cache.get('market')==None:
            self.cache.update({"market": {}})
        self.cache['market'][market_name] = id
        return id
    def load_platform(self, platform_name):
        """
        It checks if the platform name is in the cache, if not it checks if it's in
        the database, if not it inserts it into the database and returns the id
        ...
        # Cache
        if (self.cache.get('platform') != None and self.cache.get('platform').get(platform_name) != None):
            return self.cache.get('platform').get(platform_name)


        self.connect()
        query = ("""
            SELECT id FROM dim_platform WHERE platform_name=%s LIMIT 1;
        """)
        with self.conn.cursor(cursor_factory=DictCursor) as cur:
            ...
            if row:
                ...
                cur.execute(dim_platform_table_insert, (platform_name,))
                self.conn.commit()
                id = cur.fetchone()[0]
        cur.close()


        if self.cache.get('platform')==None:
            self.cache.update({"platform": {}})
        self.cache['platform'][platform_name] = id
        return id

    def load_social(self, social_name):
        if social_name is None:
            return None
        social_name = str(social_name)

        # Cache
        if (self.cache.get('social') != None and self.cache.get('social').get(social_name) != None):
            return self.cache.get('social').get(social_name)


        self.connect()
        query = ("""
            SELECT id FROM dim_social WHERE social_name=%s LIMIT 1;
        """)
        with self.conn.cursor(cursor_factory=DictCursor) as cur:
            cur.execute(query, (social_name,))
            row = cur.fetchone()
            if row:
                id = row["id"]
            else:
                cur.execute(dim_social_table_insert, (social_name,))
                self.conn.commit()
                id = cur.fetchone()[0]
        cur.close()


        if self.cache.get('social')==None:
            self.cache.update({"social": {}})
        self.cache['social'][social_name] = id
        return id
