...
def run(get_date, db_xsn, db, logs):
    ...
    query = """
            ...
            UserLoginNotPlay AS (
                ...
                FROM wallet_history
                WHERE
                    date(createdAt) = '{get_date}' and typeTransaction = 'Chơi game' and userId not in (
                        SELECT 
                            id
                        FROM users
    ...
    action_dict = {
        "name": "b3_snapshot_n1_dont_play", # Required
        "description": "B.3 N1 count that don't play", # Required
