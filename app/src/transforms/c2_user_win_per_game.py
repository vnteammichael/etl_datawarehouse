...
def run(get_date, db_xsn, db, logs):
    ...
    query = """
            WITH Users AS (
                SELECT department_id, game_id,
                    SUM(win_amount) AS metric_value
                FROM dim_user
                WHERE
                    date = '{get_date}' 
    ...
    action_dict = {
        "name": "c2_user_win_per_game", # Required
        "description": "C.2	Tiền user thắng từng game", # Required
        "metric_value": "SUM(win_amount)", # Required
        "metric_value_2": "",
        "dimension_1": "game",
        "dimension_2": "",
        "dimension_3": "",
